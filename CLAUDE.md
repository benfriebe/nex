...
- **Mermaid diagrams**: a fenced ` ```mermaid ` block (matched on the first info-string token, case-insensitive) is rendered as a live SVG in **view mode**. `MarkdownHTMLRenderer.visitMermaidBlock` emits a `<div class="mermaid-block" data-nex-mermaid>` placeholder holding the HTML-escaped source in a hidden `<pre class="mermaid-source">`, and `MarkdownRenderer.render` returns a `containsMermaid` flag (`renderToHTML` stays a `String` wrapper). The library (`Nex/Resources/mermaid.min.js`, mermaid v11.4.1 UMD, ~2.5MB, MIT) is **lazily** injected into the preview `WKWebView` via `evaluateJavaScript` only when the document contains a diagram — `MarkdownPaneView`'s `webView(_:didFinish:)` evaluates `MarkdownMermaidScript.librarySource` then `runSource(token:)`, which renders each block sequentially (mermaid keeps global parser state), reads light/dark from the live `<html>` class, and posts a `mermaidRendered` message so scroll is restored *after* the layout-changing SVGs settle (token-guarded + 5s fallback timer). A diagram that fails to parse (or a missing library) falls back to a copyable `.code-block`; an empty fence renders nothing. Find-in-page skips SVG-namespaced text so matches can't blank diagram labels. Edit mode shows the raw fence untouched.
...
- `nex` CLI — standalone Swift CLI in `Tools/nex-cli/`. Compiled as a post-build script and bundled into `Contents/Helpers/`. Subcommand structure:
  ...
  - `nex open [--here] <filepath>` — generic file opener that routes by the file's (lowercased) extension **in the CLI**, reusing existing wire commands: markdown extensions (`md`, `markdown`, `mdown`, `mkd`, `mkdn`, `mdwn`, `markdn`) send the `open` command → markdown pane; web extensions (`html`, `htm`, `pdf`, `svg`, `png`, `jpg`, `jpeg`, `gif`, `webp`) build a `file://` URL and send `web-open` → web pane (prints `open ok: <uuid>`); any other extension is a usage error pointing at `nex md` / `nex web open`. `--here` (reuse the calling pane) applies to the markdown route only — it's ignored with a stderr note for web files, which always open a new pane. The markdown/web routing tables live in `markdownOpenExtensions` / `webOpenExtensions` in `Tools/nex-cli/nex.swift`.
