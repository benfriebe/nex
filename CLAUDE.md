...
- `nex` CLI — standalone Swift CLI in `Tools/nex-cli/`. Compiled as a post-build script and bundled into `Contents/Helpers/`. Subcommand structure:
  ...
  - `nex workspace create [--name ...] [--path ...] [--color ...] [--group <name>] [--json]` — request/response (as of the `workspace delete` work): replies `{ok:true, workspace_id, workspace_name, group?}` so scripts can capture the new workspace's id (prints `created workspace <name> (<uuid>)`, or the raw reply object with `--json`). An ambiguous `--group` name is rejected with `{ok:false, error:"…ambiguous…"}` and a non-zero exit. `--group` still creates the group if it doesn't exist (unlike `workspace move`, which requires an existing group).
